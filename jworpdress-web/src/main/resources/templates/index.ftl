...
<@header title="${config.siteName?if_exists} | 追心中的海，逐世界的梦"
    keywords="${config.homeKeywords?if_exists}"
    description="${config.homeDesc?if_exists}"
    canonical="/${config.siteUrl?if_exists}">
</@header>
            ...
            <div class="scrolltext">
                <ul class="list-unstyled" id="notice-box">
                    <li class="scrolltext-title">
                        <a href="javascript:void(0)" rel="bookmark">本站正式启用新域名:<a href="http:/www.liuhaihua.cn" target="_blank"><strong>http:/www.liuhaihua.cn</strong></a></a>
                    </li>
                    <li class="scrolltext-title">
                    ...
                    <article class="fade-in">
                        <figure class="thumbnail">
                            <a href="${config.siteUrl}/posts/${item.id?c}">
                                ...
                                <#else>
                                    <img width="150" height="150" src="${config.siteUrl}/img/default_article_cover.jpg" class="img-responsive lazy-img" alt="${item.postTitle?if_exists}">
                                </#if>
                            </a>
                              <#if item.termsList?exists && (item.termsList?size > 0)>
                             	  <#list item.termsList as term>
                             	      <#if term.taxonomy?exists && (term.taxonomy=='category')>
                          				 		<span class="cat"><a href="${config.siteUrl}/type/${term.termId?if_exists}">${term.name?if_exists}</a></span>
                          			   </#if>
                          		  </#list>
                             </#if>
                        </figure>
    ...
    <ul class="list-unstyled list-inline">
        <#if indexLinkList?exists && (indexLinkList?size > 0)>
            <#list indexLinkList as item>
                <li>
                    <a href="${item.linkUrl}" target="_blank" title="${item.linkName?if_exists}" data-toggle="tooltip" data-placement="bottom">
                        <img src="${item.linkImage}" alt="${item.linkName?if_exists}" onerror="this.src='${config.siteUrl}/img/user.png'">${item.linkName?if_exists}
                    </a>
                </li>
            </#list>
            <li>
